    ...
    fn insert_inference_log(&self, entry: &super::InferenceLogEntry) {
        ...
        tx.execute(
            "INSERT INTO inference_logs (
                ...
                prompt_tokens, completion_tokens, estimated_context_tokens, total_ms, search_ms, generation_ms,
    ...
    fn list_inference_logs(
        ...
        stmt.query_map(params_refs.as_slice(), |row| {
            Ok(super::InferenceLogEntry {
                ...
                estimated_context_tokens: row.get::<_, i32>(17)? as u32,
                total_ms: row.get::<_, i64>(18)? as u64,
                search_ms: row.get::<_, Option<i64>>(19)?.map(|v| v as u64),
                generation_ms: row.get::<_, Option<i64>>(20)?.map(|v| v as u64),
                chunks_retrieved: row.get::<_, Option<i32>>(21)?.map(|v| v as u32),
                avg_chunk_score: row.get(22)?,
                self_rag_decision: row.get(23)?,
                self_rag_confidence: row.get(24)?,
                quality_guard_pass: row.get::<_, Option<i32>>(25)?.map(|v| v != 0),
                relevance_score: row.get(26)?,
                hallucination_score: row.get(27)?,
                completeness_score: row.get(28)?,
                pipeline_route: row.get(29)?,
                agents_used: row.get(30)?,
                status: row.get(31)?,
                error_message: row.get(32)?,
                response_length: row.get::<_, i32>(33)? as u32,
                feedback_score: row.get::<_, Option<i32>>(34)?.map(|v| v as i8),
