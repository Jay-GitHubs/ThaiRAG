...
impl QueryRewriter {
    ...
    pub async fn rewrite(
        ...
        let complexity_hint = match analysis.complexity {
            ...
            Complexity::Moderate => "This is a MODERATE query. Generate 1-2 sub-queries if helpful.",
            Complexity::Simple => "This is a SIMPLE query. Keep the primary query concise.",
        };

        let language_hint = match analysis.language {
            QueryLanguage::Thai => "The query is in Thai. Generate expanded_terms with English equivalents.",
            QueryLanguage::English => "The query is in English. Generate expanded_terms with Thai equivalents if relevant.",
            QueryLanguage::Mixed => "The query is mixed Thai/English. Generate expanded_terms in both languages.",
        };
        ...
        let system = ChatMessage {
            ...
            content: self.prompts.render_or_default(
                "chat.query_rewriter",
                DEFAULT_TEMPLATE,
                &[("complexity_hint", complexity_hint), ("language_hint", language_hint)],
            ),
        };
            ...
            Ok(resp) => {
                ...
                match serde_json::from_str::<LlmRewrite>(json_str) {
                    Ok(r) => {
                        ...
                        Ok(RewrittenQueries {
                            primary: if r.primary.is_empty() { query.to_string() } else { r.primary },
    ...
    pub async fn rewrite_with_feedback(
        ...
        let user = ChatMessage {
            role: "user".into(),
            content: format!("Original query: {query}\nLanguage: {:?}\nComplexity: {:?}", analysis.language, analysis.complexity),
        };
