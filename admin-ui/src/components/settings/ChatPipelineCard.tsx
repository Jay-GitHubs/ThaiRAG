...
export function ChatPipelineCard({ scope }: { scope?: SettingsScopeParam }) {
  ...
  const [minVectorRelevance, setMinVectorRelevance] = useState(0.3);
  ...
  async function loadConfig() {
    try {
      ...
      setMinVectorRelevance(data.min_vector_relevance ?? 0.3);
      ...
      {!enabled ? (
        ...
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          ...
          <Space size="large" wrap>
            ...
            <Tooltip title="No-context refusal floor. The best retrieved chunk's dense-vector cosine (0–1) must reach this to attempt an answer; below it, the question is treated as out-of-domain and refused with a 'No answer' result instead of answering from irrelevant context. Calibrated default 0.30 (out-of-domain/absent-doc queries measure ~0.24–0.27, legit in-corpus queries ~0.35+). Set 0 to disable. Tune from the 'retrieval_vector_score' logged per request.">
              <Space direction="vertical" size={2}>
