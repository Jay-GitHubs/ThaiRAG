...
/** Back-compat shim for code that still uses the old light/dark switch. Maps the
 *  two-state toggle onto the default light theme (Celadon) and default dark (Ink). */
// eslint-disable-next-line react-refresh/only-export-components
export function useThemeMode() {
  const { theme, setTheme } = useTheme();
  return {
    mode: (theme.dark ? 'dark' : 'light') as 'light' | 'dark',
    toggle: () => setTheme(theme.dark ? 'celadon' : 'ink'),
  };
}
