...
test.describe('KM Hierarchy', () => {
  ...
  test('full CRUD: create org → dept → workspace → delete all', async ({ page, request }) => {
    // Long multi-step UI flow with several post-mutation tree refreshes; give it
    // headroom beyond the 30s global cap so relaxed waits don't blow the budget.
    ...
    await expect(createOrgModal).not.toBeVisible({ timeout: 15000 });

    // Org should appear in the tree
    await expect(page.locator('.ant-tree').getByText(orgName)).toBeVisible({ timeout: 15000 });

    // --- Select Org → OrgPanel ---
    await page.locator('.ant-tree').getByText(orgName).click();
    await expect(page.getByText(`Organization: ${orgName}`)).toBeVisible({ timeout: 15000 });
    ...
    await expect(createDeptModal).not.toBeVisible({ timeout: 15000 });

    // Dept appears in the OrgPanel's departments table
    await expect(page.getByRole('cell', { name: deptName })).toBeVisible({ timeout: 15000 });
    ...
    await expect(page.locator('.ant-tree').getByText(deptName)).toBeVisible({ timeout: 15000 });
    await page.locator('.ant-tree').getByText(deptName).click();
    await expect(page.getByText(`Department: ${deptName}`)).toBeVisible({ timeout: 15000 });
    ...
    await expect(createWsModal).not.toBeVisible({ timeout: 15000 });

    // Workspace appears in the DeptPanel's workspaces table
    await expect(page.getByRole('cell', { name: wsName })).toBeVisible({ timeout: 15000 });
    ...
    await expect(page.getByRole('cell', { name: wsName })).not.toBeVisible({ timeout: 15000 });
    ...
    await page.locator('.ant-popconfirm').getByRole('button', { name: 'OK' }).click();
    await expect(page.locator('.ant-tree').getByText(orgName)).not.toBeVisible({ timeout: 15000 });
  });
});
